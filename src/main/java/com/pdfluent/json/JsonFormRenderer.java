...
public class JsonFormRenderer {
    ...
    public static Document render(JsonObject root) {
        return render(root, PageSettings.a4().margin(50).build());
    }
    ...
    private static void renderField(ContentBuilder page, JsonObject field) {
        ...
        switch (type) {
            ...
            // Signature
            case "SIGNATURE"    -> renderSignatureField(page, data);
    ...
    private static void renderSignatureField(ContentBuilder page, JsonObject data) {
        boolean hasSig = !getString(data, "plainValue", "").isEmpty();

        page.spacer(4);
        if (hasSig) {
            page.text("[Signed]", tc -> tc.italic().fontSize(VALUE_FONT_SIZE)
                    .color(VALUE_COLOR).spaceAfter(2));
        }
